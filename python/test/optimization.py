...
class TestProblemDefinition(unittest.TestCase):
    ...
    def test_problem_definition(self):
        ...
        pD.init_pos
        pD.init_vel = matrix([0.0, 1.0, 1.0]).transpose()
        pD.end_vel = matrix([0.0, 1.0, 1.0]).transpose()
        pD.init_acc = matrix([0.0, 1.0, -1.0]).transpose()
        pD.end_acc = matrix([0.0, 0.0, 0]).transpose()
