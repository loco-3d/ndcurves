...
struct problem_data {
  ...
  std::size_t numControlPoints;  // total number of control Points (variables +
                                 // waypoints) / DIM )
  ...
  problem_data(const problem_data& other)
    ...
    bezier = new bezier_t(b.waypoints().begin(), b.waypoints().end(), b.T_min_,
                          b.T_max_, b.mult_T_);
  }
};
...
Bezier* compute_linear_control_points(const problem_data<Point, Numeric>& pData,
  ...
  for (std::size_t i = 0; i < totalvar; ++i)
    res.push_back(fill_with_zeros<Numeric, LinearVar>(
        linearVars[i], i, pData.startVariableIndex, pData.numVariables,
        pData.dim_));
  return new Bezier(res.begin(), res.end(), 0., totalTime);
}

template <typename Point, typename Numeric, bool Safe>
problem_data<Point, Numeric, Safe> setup_control_points(
  ...
  if (numActiveConstraints >= numControlPoints)
    throw std::runtime_error(
        "In setup_control_points; too many constraints for the considered "
        "degree");
  ...
  if (flag & INIT_POS) {
    ...
    if (flag & INIT_VEL) {
      point_t vel =
          pDef.init_pos + (pDef.init_vel / (num_t)degree) / pDef.totalTime;
      ...
      if (flag & INIT_ACC) {
        ...
        ++i;
        if (flag & INIT_JERK) {
          point_t jerk = pDef.init_jerk * pDef.totalTime * pDef.totalTime *
                             pDef.totalTime /
  ...
  if (flag & END_POS) {
    if (flag & END_VEL) {
      point_t vel =
          pDef.end_pos - (pDef.end_vel / (num_t)degree) / pDef.totalTime;
      if (flag & END_ACC) {
        point_t acc = (pDef.end_acc / (num_t)(degree * (degree - 1))) /
                          (pDef.totalTime) * (pDef.totalTime) +
                      2 * vel - pDef.end_pos;
        if (flag & END_JERK) {
          point_t jerk = -pDef.end_jerk * pDef.totalTime * pDef.totalTime *
                             pDef.totalTime /
                             (num_t)(degree * (degree - 1) * (degree - 2)) +
  ...
  problemData.numStateConstraints =
      numActiveConstraints - problemData.numVariables;
  problemData.bezier = compute_linear_control_points<
      Point, Numeric, bezier_curve<Numeric, Numeric, true, var_t>, var_t>(
...
long compute_num_ineq_control_points(
  ...
  for (typename problem_definition_t::CIT_vector_x_t cit =
           pDef.inequalityVectors_.begin();
...
template <typename Point, typename Numeric>
std::vector<bezier_curve<Numeric, Numeric, true, linear_variable<Numeric> > >
split(const problem_definition<Point, Numeric>& pDef,
...
template <typename Point, typename Numeric>
void initInequalityMatrix(const problem_definition<Point, Numeric>& pDef,
                          problem_data<Point, Numeric>& pData,
  ...
  if (pDef.inequalityMatrices_.size() != beziers.size()) {
    throw std::invalid_argument(
        "The sizes of the inequality matrices and the bezier degree do not "
        "match.");
  }

  long currentRowIdx = 0;
  typename problem_definition_t::CIT_matrix_x_t cmit =
      pDef.inequalityMatrices_.begin();
  typename problem_definition_t::CIT_vector_x_t cvit =
      pDef.inequalityVectors_.begin();
  // for each bezier split ..
  for (CIT_bezier_t bit = beziers.begin(); bit != beziers.end();
       ++bit, ++cvit, ++cmit) {
    ...
    for (cit_point cit = wps.begin(); cit != wps.end(); ++cit) {
      prob.ineqMatrix.block(currentRowIdx, 0, cmit->rows(), cols) =
          (*cmit) * (cit->B());  // constraint inequality for current bezier *
                                 // expression of control point
      prob.ineqVector.segment(currentRowIdx, cmit->rows()) =
          *cvit - (*cmit) * (cit->c());
      currentRowIdx += cmit->rows();
    }
  }
  assert(rows == currentRowIdx);  // we filled all the constraints - NB: leave
                                  // assert for Debug tests
}

template <typename Point, typename Numeric, typename In>
quadratic_variable<Numeric> bezier_product(In PointsBegin1, In PointsEnd1,
                                           In PointsBegin2, In PointsEnd2,
                                           const std::size_t /*Dim*/) {
  ...
  if (nPoints1 <= 0 || nPoints2 <= 0) {
    throw std::runtime_error(
        "This should never happen because an unsigned int cannot go negative "
        "without underflowing.");
  }
  unsigned int deg1 = nPoints1 - 1, deg2 = nPoints2 - 1;
  unsigned int newDeg = (deg1 + deg2);
  // the integral of the primitive will simply be the last control points of the
  // primitive, divided by the degree of the primitive, newDeg. We will store
  // this in matrices for bilinear terms, and a vector for the linear terms, as
  // well as another one for the constants.
  ...
  // is either the fraction given by  (i+j)/ (deg1+deg2), or 1 - (i+j)/
  // (deg1+deg2). The trick is that the condition is given by whether the
  // current index in the combinatorial is odd or even. time parametrization is
  // not relevant for the cost

  Numeric ratio;
  for (unsigned int i = 0; i < newDeg + 1; ++i) {
    unsigned int j = i > deg2 ? i - deg2 : 0;
    for (; j < std::min(deg1, i) + 1; ++j) {
      ratio = (Numeric)(bin(deg1, j) * bin(deg2, i - j)) /
