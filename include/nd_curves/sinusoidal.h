...
struct sinusoidal : public curve_abc<Time, Numeric, Safe, Point> {
 ...
 public:
  ...
  virtual bool isApprox(const sinusoidal_t& other,
    ...
    return nd_curves::isApprox<time_t>(T_min_, other.min()) && nd_curves::isApprox<time_t>(T_max_, other.max()) &&
           dim_ == other.dim() && p0_.isApprox(other.p0_, prec) && amplitude_.isApprox(other.amplitude_, prec) &&
           nd_curves::isApprox<time_t>(T_, other.T_) && nd_curves::isApprox<time_t>(phi_, other.phi_);
  }
...
DEFINE_CLASS_TEMPLATE_VERSION(SINGLE_ARG(typename Time, typename Numeric, bool Safe, typename Point),
                              SINGLE_ARG(nd_curves::sinusoidal<Time, Numeric, Safe, Point>))
